...
std::vector<std::string> pre2_post_cxx_re(schedulePtr thisTarget, schedulePtrVecType schedule){
    ...
    for (const auto& post : thisTarget->postCourse){
        auto resFromRe = pre2_post_cxx_re(post, schedule);
...
std::map<std::string, std::vector<std::string>> pre2post_cxx(schedulePtrVecType schedule){
    ...
    for (auto i : schedule){
        ...
        auto post = pre2_post_cxx_re(i, schedule);
        // Remove this course
